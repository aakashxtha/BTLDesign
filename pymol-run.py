...
def generate_pymol_script(run_number):
    ...
    # Filter mutations for the specified run
    run_mutations = {k: v for k, v in mutations.items() if f"_design_{run_number}_" in k}
    ...
    for pdb_file in run_mutations.keys():
        if pdb_file in pdb_files:
    ...
    for pdb_file, mutation_list in run_mutations.items():
        if pdb_file in pdb_files:
            design_name = os.path.splitext(pdb_file)[0]
    ...
    output_file = f'align_and_show_mutations_run_{run_number}.pml'
    with open(output_file, 'w') as f:
        f.write('\n'.join(commands))
    
    print(f"PyMOL script for run {run_number} has been generated: {output_file}")

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python pymol-script-generator.py <run_number>")
        sys.exit(1)
    
    run_number = sys.argv[1]
    generate_pymol_script(run_number)